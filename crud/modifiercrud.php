...
if (isset($_POST['modification'])) {
  if (isset($_GET['id'])) {
    ...
    $email=$_POST['Email'];
    $db = new PDO('mysql:host=localhost;dbname=train;charset=utf8', 'root', 'root');
    $change = $db->prepare(" UPDATE  user SET name =:name , role = :role ,Email=:Email WHERE id = $id");
    $change->execute(
      array(
        ':name' => $name,
        ':role' => $role,
    ...
    <body>
      ...
      <form action="" method="post">
        <div class="container">
          ...
          <input type="text" class="form-control" name="role" value="<?php echo $userid['role'] ?>" ;>
          <label for="email" class="form-label"><strong>Email</strong></label>
          <input type="email" class="form-control" name="Email" value="<?php echo $userid['Email'] ?>" ;>
