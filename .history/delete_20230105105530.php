...
if(isset($_GET["messageid"]) && $_GET["messageid"] <> $_SESSION['user']){
...
// nous allons se connecter a la base de donnés , en executant un requête qui va selectionner l'attribut  
    ...
    $sel = "SELECT * FROM messages WHERE id = $idmessage";
 ...
 * première verification est de verfier si l'utilisateur est connecté
 * deuxième verifcation est de verifier si le messageid est renseigne et present en url
 * 3eme verifcation est de verifier si le message id   existe en bdd
 * verifier si l'auteur du message correspond au user connecté
 */
